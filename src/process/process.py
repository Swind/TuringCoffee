...
class Unit(object):
    ...
    __from_to = re.compile(
        '([0-9\.]+)\s*([a-z]+)(\s*to\s*(([0-9\.]+)\s*([a-z]+))?)?')
    ...
    def __from_to_handler(self, unit_and_factory, groups):
        from_value = self.__number_with_unit_handler(
            unit_and_factory, (groups[0], groups[1]))
        to_value = self.__number_with_unit_handler(
            unit_and_factory, (groups[4], groups[5]))
            ...
            for value in values:
                ...
                if except_unit is None:
                    ...
                    if m is None:
                        ...
                        value_result = unit_item['handler'](
                            unit_item['unit_and_factory'], m.groups())
