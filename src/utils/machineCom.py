"""MachineCom handles communication with GCode based printers trough (USB)
serial ports.

For actual printing of objects this module is used from
Cura.serialCommunication and ran in a separate process.
...
class MachineCom(object):
    ...
    def _changeState(self, newState):
        ...
        self._log('Changing monitoring state from \'%s\' to \'%s\'' %
                  (oldState, self.getStateString()))
    ...
    def getPrintTimeRemainingEstimate(self):
        ...
        printTimeTotal = printTime * \
            (len(self._gcodeList) - 100) / (self.getPrintPos() - 100)
    ...
    def _try_to_open_serial(self):
        try:
            if self._port == 'VIRTUAL':
                ...
                self._changeState(self.STATE_OPEN_SERIAL)
                self._log('Connecting to: %s with baudrate: %s (configured)' % (
            ...
            self._log('Unexpected error while connecting to serial port: %s %s' % (
    ...
    def _handle_error_result(self, line):
        ...
        # So we can have an extra newline in the most common case. Awesome work
        # people.
    ...
    def _handle_temperature_result(self, line):
        ...
        if 'B:' in line:
            ...
            self._callback.mcTempUpdate(
                self._temp, self._bedTemp, self._targetTemp, self._bedTargetTemp)

            # If we are waiting for an M109 or M190 then measure the time we
            # lost during heatup, so we can remove that time from our printing
            # time estimate.
    ...
    def _monitor(self):
        while True:
            ...
            if line is None:
                self._log(
                    'The line read from arduino is None, connection closed')
                self.close()
                continue
            ...
            if self._state == self.STATE_CONNECTING:
                # 'wait' needed for Repetier (kind of watchdog)
                ...
                # Request the temperature on comm timeout (every 2 seconds)
                # when we are not printing.
                if line == '':
                    if self._extruderCount > 0:
                        self._temperatureRequestExtruder = (
                            self._temperatureRequestExtruder + 1) % self._extruderCount
                        self.sendCommand(
                            'M105 T%d' % (self._temperatureRequestExtruder))
    ...
    def _readline(self):
        ...
        try:
            ...
            self._log('Unexpected error while reading serial port: %s' %
                      (getExceptionString()))
    ...
    def _sendCommand(self, cmd):
        ...
        if 'M104' in cmd or 'M109' in cmd:
            try:
                ...
                self._targetTemp[t] = float(
                    re.search('S([0-9]+)', cmd).group(1))
            except:
                pass
        if 'M140' in cmd or 'M190' in cmd:
            try:
                self._bedTargetTemp = float(
                    re.search('S([0-9]+)', cmd).group(1))
            except:
                pass
        ...
        try:
            ...
            try:
                ...
                self._log(
                    'Unexpected error while writing serial port: %s' % (getExceptionString()))
                self._errorValue = getExceptionString()
                self.close(True)
        except:
            self._log('Unexpected error while writing serial port: %s' %
                      (getExceptionString()))
            self._errorValue = getExceptionString()
            self.close(True)
    ...
    def _sendNext(self):
        ...
        try:
            if line == 'M0' or line == 'M1':
                # self.setPause(True)
                # Don't send the M0 or M1 to the machine, as M0 and M1 are
                # handled as an LCD menu pause.
                line = 'M105'
            if self._printSection in self._feedRateModifier:
                line = re.sub('F([0-9]*)', lambda m: 'F' + str(
                    int(int(m.group(1)) * self._feedRateModifier[self._printSection])), line)
...
class VirtualPrinter():
    ...
    """A virtual printer class used for debugging.

    Acts as a serial.Serial class, but without connecting to any port.
