...
The STK500v2 protocol is used by the ArduinoMega2560 and a few other
Arduino platforms to load firmware.
...
class Stk500v2(ispBase.IspBase):
    ...
    def connect(self, port='COM22', speed=115200):
        ...
        try:
            self.serial = Serial(
                str(port), speed, timeout=1, writeTimeout=10000)
        except SerialException as e:
            raise ispBase.IspError('Failed to open serial port')
        except:
            raise ispBase.IspError(
                'Unexpected error while connecting to serial port:' + port + ':' + str(sys.exc_info()[0]))
        self.seq = 1
    ...
    def writeFlash(self, flashData):
        # Set load addr to 0, in case we have more then 64k flash we need to
        # enable the address extension
        ...
        for i in xrange(0, loadCount):
            recv = self.sendMessage([0x13, pageSize >> 8, pageSize & 0xFF, 0xc1, 0x0a, 0x40,
                                     0x4c, 0x20, 0x00, 0x00] + flashData[(i * pageSize):(i * pageSize + pageSize)])
    ...
    def verifyFlash(self, flashData):
        if self._has_checksum:
            self.sendMessage([0x06, 0x00, (len(flashData) >> 17) & 0xFF, (len(
                flashData) >> 9) & 0xFF, (len(flashData) >> 1) & 0xFF])
            ...
            if hex(checksum) != hex(checksum_recv):
                raise ispBase.IspError(
                    'Verify checksum mismatch: 0x%x != 0x%x' % (checksum & 0xFFFF, checksum_recv))
        else:
            # Set load addr to 0, in case we have more then 64k flash we need
            # to enable the address extension
...
def runProgrammer(port, filename):
    """Run an STK500v2 program on serial port 'port' and write 'filename' into
    flash."""
    programmer = Stk500v2()
...
def main():
    ...
    if sys.argv[1] == 'AUTO':
        ...
        for port in portList():
            threading.Thread(
                target=runProgrammer, args=(port, sys.argv[2])).start()
            time.sleep(5)
    else:
        programmer = Stk500v2()
